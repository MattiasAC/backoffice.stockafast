...
class PDF extends Fpdi
{
    ...
    public function generate()
    {
        ...
        $numItems = count($this->inv->meters);
        $w = ($numItems <= 2) ? ($numItems == 1 ? 35 : 18) : 18;
        $col = 67;

        $meterCount = 1;
        $maxY = 0;
        $metersData = [];
        foreach ($this->inv->meters as $meter) {
            $meterid = $meter['meterid'];
            $metersData[$meterid] = ['Name' => $meter['name'], 'List' => $this->inv->measurementsByMeter[$meterid] ?? []];
        }
        ...
        foreach ($metersData as $Meter) {
            ...
            $this->printEl($Meter['List'], $Meter['Name'], [$w, $w, $w, $w]);
            $maxY = max($maxY, $this->getY());
            if ($meterCount < $numItems) {
                $this->setY(10);
                $this->setLeftMargin($meterCount * $col + 10);
                $this->setX($meterCount * $col + 10);
            } else {
                $this->setLeftMargin(10);
                $this->setY($maxY);
                $this->ln();
        ...
        $this->printSummery();
        $this->ln();
        $this->printInvoiced($this->inv->invoicings);

        $one = false;
        if (strpos($this->inv->elavtal, "spot") !== false && !$one) {
            $this->ln();
            $extra = str_replace("spot+", "", $this->inv->elavtal);
            $this->printSpot($this->inv->spot, $extra);
            $one = true;
        }

        $path = "./html/el/pdf/{$this->inv->fileName}";
        $this->Output($path, "F");
    }
